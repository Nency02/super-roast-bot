...
from rate_limiter import rate_limiter, WINDOW_SECONDS
...
if user_input := st.chat_input("Say something... if you dare 🔥"):
    ...
    # Rate limit check using session ID (compatible with future user/IP-based auth)
    client_id = st.session_state.session_id
    if not rate_limiter.is_allowed(client_id):
        retry_after = rate_limiter.get_retry_after(client_id)
        reply = f"🚫 Rate limit exceeded. Too many requests — try again in {retry_after} seconds. (Limit: {rate_limiter.max_requests} requests per {WINDOW_SECONDS}s)"
